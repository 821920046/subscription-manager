...
### 修复：免费套餐 KV list 限额超出（回归）
- **病因**：v2.2.0 将订阅读取改为 KV `list()`；而定时任务每分钟触发（`* * * * *`）会调用 `getAllSubscriptions()` → 每天 ~1440 次 list，远超免费套餐的 **每日 1000 次 list** 上限，导致持续 429 告警。（免费额度：get 10 万/天、list 仅 1000/天）
- **修复**：读取热路径改回基于索引的 `get`（索引 + 各条目），**不再在热路径使用 list**；仅当索引缺失（首次/迁移）时用一次 list 重建并持久化。创建/删除同步维护索引。
- **自愈**：新增 `POST /api/reindex`（需登录），若索引与实际数据漂移可手动重建（单次仅 1 次 list）。
- **迁移**：从 v2.2.x（无索引）升级后，首次读取会自动用一次 list 重建索引，之后即零 list。
- **验证**：新增 KV 操作计数测试，证明定时任务路径在索引存在时 **0 次 list**；创建/删除/读取均不触发 list。

> 提示：若你不需要“分钟级”提醒精度，可将 `wrangler.toml` 的 `crons` 改为例如每 5 分钟 `"*/5 * * * *"` 以进一步降低读取次数（非必需，读取额度充足）。
