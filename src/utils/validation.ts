...
export const SubscriptionSchema = z.object({
    ...
    name: z.string().min(1, '服务名称不能为空').max(CONFIG.VALIDATION.MAX_NAME_LENGTH, `服务名称不能超过 ${CONFIG.VALIDATION.MAX_NAME_LENGTH} 字符`),
    ...
    periodValue: z.number().int().min(CONFIG.VALIDATION.MIN_PERIOD_VALUE).max(CONFIG.VALIDATION.MAX_PERIOD_VALUE).optional().default(CONFIG.DEFAULTS.PERIOD_VALUE),
    ...
    reminderDays: z.number().int().min(CONFIG.VALIDATION.MIN_REMINDER_DAYS).max(CONFIG.VALIDATION.MAX_REMINDER_DAYS).optional().default(CONFIG.DEFAULTS.REMINDER_DAYS),
    dailyReminderTimes: z.array(z.string().regex(timeFormatRegex, '时间格式必须为 HH:mm')).optional(),
    notes: z.string().max(CONFIG.VALIDATION.MAX_NOTES_LENGTH, `备注不能超过 ${CONFIG.VALIDATION.MAX_NOTES_LENGTH} 字符`).optional().default(''),
...
export const TestNotificationSchema = z.object({
    type: z.enum(['telegram', 'notifyx', 'wenotify', 'webhook', 'wechatbot', 'wechatOfficialAccount', 'email', 'bark']),
}).passthrough(); // 允许额外字段用于各渠道配置
