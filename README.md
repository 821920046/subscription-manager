...
- **智能多路推送**：支持对不同订阅设置独立的推送目标。
  - **WeNotify Edge**：支持多用户分发（由 `|` 分隔）及订阅级定向。
  - **企业微信机器人**：支持多 Webhook 及根据 Webhook Key 进行订阅级匹配推送。
  - **邮件通知**：支持多收件人及订阅级收件人设置。
...
- **智能通知分发系统 (2026-02-01)**：
  - **WeNotify 多用户分发**：全局配置支持多用户 ID，支持在添加/编辑订阅时指定 `weNotifyUserIds`，实现定向推送。
  - **企微机器人多路推送**：全局支持多个 Webhook URL，并根据订阅配置的 `wechatBotKeys` 与 Webhook URL 中的 `key` 参数进行智能匹配。
  - **邮件消息定向发送**：支持配置多个全局邮箱，并允许在订阅级别覆盖收件地址。
  - **Admin UI 升级**：订阅编辑弹窗新增“WeNotify 定向推送用户”、“企微机器人 Key”以及“邮件通知地址”配置字段，并修复了日期保存时的格式校验问题。
...
3. **添加订阅**：
   ...
   - 设置提醒提前天数（如提前 7 天提醒）。
   - **智能推送配置 (定向推送)**：
     - **WeNotify**：在“WeNotify 定向推送用户”中填入用户 ID (如 `UserA|UserB`)，该订阅将仅推送给这些用户。
     - **企微机器人**：在“企微机器人 Key”中设置匹配标识，系统将自动匹配包含对应 Key 的 Webhook 进行推送。
     - **邮件通知**：在“邮件通知地址”中填入特定邮箱，覆盖全局默认设置。
     - *注：若留空，则默认推送到该渠道下配置的所有全局地址。*
